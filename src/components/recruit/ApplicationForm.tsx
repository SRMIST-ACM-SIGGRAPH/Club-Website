...
export function ApplicationForm() {
  ...
  const validateStep = (currentStep: number) => {
    ...
    if (currentStep === 3) {
      ...
      const wordCount = formData.why_join.trim().split(/\s+/).filter(w => w.length > 0).length;
      if (wordCount < 50 || wordCount > 100) {
        newErrors.why_join = `Must be between 50 and 100 words (Currently: ${wordCount})`;
        isValid = false;
      }
    }
    ...
    <div className="max-w-2xl mx-auto mt-16 p-6 md:p-8 bg-[#050505]/80 backdrop-blur-md border border-white/10 rounded-2xl relative overflow-hidden">
      {!user ? (
        <div className="text-center py-12">
          ...
          <button 
            onClick={handleLogin}
            className="px-6 py-3 bg-white text-black font-bold rounded-lg hover:bg-gray-200 transition-colors flex items-center justify-center mx-auto gap-3"
          >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <div className="flex justify-between items-end mb-1">
                      ...
                      <span className={`text-xs ${formData.why_join.trim().split(/\s+/).filter(w=>w.length>0).length < 50 || formData.why_join.trim().split(/\s+/).filter(w=>w.length>0).length > 100 ? 'text-red-400' : 'text-green-400'}`}>
                        {formData.why_join.trim().split(/\s+/).filter(w=>w.length>0).length} / 50-100 words
                      </span>
                    </div>
                    <textarea 
                      rows={5} 
